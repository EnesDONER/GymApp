...
const resizeImages = tryCatch(async (req, res, next) => {
    ...
    await Promise.all(req.files.map(async (file, i) => {
        ...
        if (result.length > 0) {
            ...
            const file_name = `/content/image-${Date.now()}-${random}.png`;
            req.body.images.push({filedname:file.fieldname,filename:process.env.DOMAIN+file_name})
